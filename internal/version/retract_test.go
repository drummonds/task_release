...
`
	...
	want := []Version{{Major: 1, Minor: 37, Patch: 6}, {Major: 1, Minor: 37, Patch: 5}, {Major: 1, Minor: 37, Patch: 4}}
	if len(got) != len(want) {
...
`
	...
	if len(got) != 1 || got[0] != (Version{Major: 0, Minor: 1, Patch: 9}) {
...
func TestLatestFromTagsWithRetracted(t *testing.T) {
	...
	retracted := []Version{{Major: 1, Minor: 37, Patch: 4}, {Major: 1, Minor: 37, Patch: 5}, {Major: 1, Minor: 37, Patch: 6}}

	got, found := LatestFromTags(tags, retracted)
	if !found {
		t.Fatal("expected to find a version")
	}
	want := Version{Major: 0, Minor: 48, Patch: 12}
	if got != want {
...
func TestBumpPastRetracted(t *testing.T) {
	v := Version{Major: 0, Minor: 1, Patch: 8}
	retracted := []Version{{Major: 0, Minor: 1, Patch: 9}, {Major: 0, Minor: 1, Patch: 10}}
	got := v.BumpPastRetracted(retracted)
	want := Version{Major: 0, Minor: 1, Patch: 11}
	if got != want {
...
func TestBumpPastRetractedNone(t *testing.T) {
	...
	want := Version{Major: 0, Minor: 1, Patch: 9}
	if got != want {
		t.Errorf("BumpPastRetracted() = %v, want %v", got, want)
	}
}
